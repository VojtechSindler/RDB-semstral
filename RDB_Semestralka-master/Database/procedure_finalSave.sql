...
SET @countPoint = (SELECT COUNT(pointID) FROM Points Where pointID = @IDpoint2 AND X=@X2 AND Y=@Y2);
-----------------------------------------------------------------------------------------------------------------------------
	if(@countPoint = 0)
		BEGIN			
		...
		BEGIN
			if(@idVar!=0 AND @idGroups!=0 AND @idMach_check!=0)
				BEGIN
					...
					INSERT INTO Measurement ([time],pointID,X,Y,machineID,groupID) VALUES (@time2,@IDpoint2,@X2,@Y2,@IDmachine2,@IDgroup2)
...
@time = '11111111111', @IDpoint = 30006, @descPoint = 'Pokusny bod85', @X = 12.8247, @Y = 13.24, 
@pointValue1 = 122,@pointValue2 = 222,
@variableName = 'odpor87';*/
/*go
SELECT * from Multiple_data_select ORDER By pointID DESC;
